...
public class QuickSort extends Application {
	 ...
	 * Sets up the graphics context, creates the renderer, and begins the animation timer
	...
	private void initialize() {
		...
		renderer = new Renderer(gc, canvasWidth, canvasHeight,new Color(.3, .6, .9, 1),BACKGROUND_COLOR, soundEnabled);
	 ...
	 * Clears the current visualization, generates new data, and starts the selected sorting algorithm.
	 */
	private void restart(boolean generateNewArray) {
        ...
        else renderer.copyRenderArrayOfValues(arrayOfValuesToSort);

		for (int a = 0; a < arrayOfValuesToSort.length; a++) {
			renderer.rectangleToScreen(a, (canvasHeight / Renderer.PIXEL_SIZE) - arrayOfValuesToSort[a], 1,arrayOfValuesToSort[a], true, true);
		}
	...
	/**
	 * Swaps two elements in the array and adds the swap action to the renderer queue.
	 * Also plays a sound based on the distance between the swapped elements.
	 * Low frequency (bass) for nearby elements, high frequency for distant elements.
	 * 
	...
	private void swap(int ini, int fin) {
		...
		Renderer.actionQueue.add(new Renderer.Action(Action.SWAP, ini, fin));
    ...
    private void selectionSort(int ini, int fin) {
        ...
        for (int i = ini; i < fin; i++) {
            ...
            int minIndex = i;
            Renderer.actionQueue.add(new Renderer.Action(Action.PIVOT, i, 0));
    ...
    private void insertionSort(int ini, int fin) {
        // Start from the second element (index ini+1) as the first element is considered sorted
     ...
     * 
     * An optimized version is implemented that stops early if no swaps are made in a pass,
	...
	private void quickSort(int ini, int fin) {
		...
		Renderer.actionQueue.add(new Renderer.Action(Action.PIVOT, fin, 0));
		...
		while (i <= j) {
			...
			while (i <= j && arrayOfValuesToSort[i] <= pivotValue) i++;
			// Find element from right that should be on left side
			while (i <= j && arrayOfValuesToSort[j] > pivotValue) j--;
		...
		if (arrayOfValuesToSort[i] > pivotValue) {
			swap(i, fin);
		}
